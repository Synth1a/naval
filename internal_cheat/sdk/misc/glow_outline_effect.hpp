...
public:
	...
	public:
		...
		// @note:
		// maybe use it later
		void Set(float* color, float bloom, int style) {
			m_vGlowColor = Vector(color[0], color[1], color[2]);
			m_flGlowAlpha = color[3];
			m_bRenderWhenOccluded = true;
			m_bRenderWhenUnoccluded = false;
			m_flBloomAmount = bloom;
			m_nGlowStyle = style;
		...
		void Set(Color color, float bloom, int style) {
			m_vGlowColor = Vector(color.r() / 255.0f, color.g() / 255.0f, color.b() / 255.0f);
			m_flGlowAlpha = color.a() / 255.0f;
			m_bRenderWhenOccluded = true;
			m_bRenderWhenUnoccluded = false;
			m_flBloomAmount = bloom;
			m_nGlowStyle = style;
