...
void shots::register_shot(player_t* target, Vector eye_pos, lagcompensation::LagRecord_t* record, int fire_tick, HitscanPoint_t point, bool safe)
{
	...
	if (target)
	{
		AimPlayer* data = &aimbot::get().m_players[target->EntIndex()];
		++data->m_shots;
	}
}
...
void shots::on_fsn()
{
	...
	auto latency = networking::get().latency + 1.0f;
	...
	auto data = &aimbot::get().m_players[current_shot->target->EntIndex()];
...
void shots::on_impact(Vector impactpos)
{
	...
	if (!current_shot->record->m_bIsFakePlayer && current_shot->shot_info.safe)
	{
		...
		std::memcpy(current_shot->target->m_CachedBoneData().Base(), current_shot->record->m_pMatrix.left, current_shot->target->m_CachedBoneData().Count() * sizeof(matrix3x4_t)); //-V807
		...
		std::memcpy(current_shot->target->m_CachedBoneData().Base(), current_shot->record->m_pMatrix.right, current_shot->target->m_CachedBoneData().Count() * sizeof(matrix3x4_t)); //-V807
		...
		std::memcpy(current_shot->target->m_CachedBoneData().Base(), current_shot->record->m_pMatrix.center, current_shot->target->m_CachedBoneData().Count() * sizeof(matrix3x4_t)); //-V807
...
void shots::on_player_hurt(IGameEvent* event, int user_id)
{
	...
	if (weapon_is_aim(weapon))
	{
		...
		otheresp::get().hitmarker.point = entity->hitbox_position_matrix(util::get_hitbox_by_hitgroup(hitgroup), current_shot && entity == current_shot->target ? current_shot->record->m_pMatrix.main : entity->m_CachedBoneData().Base());

		otheresp::get().damage_marker[user_id] = otheresp::Damage_marker
		{
			entity->hitbox_position_matrix(util::get_hitbox_by_hitgroup(hitgroup), current_shot && entity == current_shot->target ? current_shot->record->m_pMatrix.main : entity->m_CachedBoneData().Base()),
			m_globals()->m_curtime,
			Color::White,
			damage,
			hitgroup
