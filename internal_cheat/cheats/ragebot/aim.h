...
public:
	...
	void SetupHitboxes(lagcompensation::LagRecord_t* record, bool history);
	bool SetupHitboxPoints(lagcompensation::LagRecord_t* record, matrix3x4_t* bones, int index, std::vector< HitscanPoint_t >& points);
	bool GetBestAimPosition(HitscanPoint_t& point, float& damage, int& hitbox, bool& safe, lagcompensation::LagRecord_t* record, float& min_damage);
...
public:
	...
	HitscanPoint_t     m_point;
	float      m_damage;
	lagcompensation::LagRecord_t* m_record;
	bool	   m_safe;
...
public:
	...
	bool CanHit(Vector start, Vector end, lagcompensation::LagRecord_t* record, int box, bool in_shot = false, matrix3x4_t* bones = nullptr);
public:
	// aimbot.
	void init();
	...
	float CheckHitchance(player_t* player, const Vector& angle, lagcompensation::LagRecord_t* record, int hitbox);
	bool SelectTarget(lagcompensation::LagRecord_t* record, const HitscanPoint_t& point, float damage);
	...
	bool IsSafePoint(lagcompensation::LagRecord_t* LagRecord, Vector vecStartPosition, Vector vecEndPosition, int iHitbox);
	// knifebot.
	void knife(CUserCmd* m_pcmd);
	bool CanKnife(lagcompensation::LagRecord_t* record, Vector angle, bool& stab);
	bool KnifeTrace(Vector dir, bool stab, CGameTrace* trace);
	bool KnifeIsBehind(lagcompensation::LagRecord_t* record);
};