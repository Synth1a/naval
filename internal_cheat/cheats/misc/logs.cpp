...
void eventlogs::paint_traverse()
{
	...
	{
		float in_anim = math::clamp((g_ctx.globals.absolute_time() - logs[i].log_time) / animation_time, 0.01f, 1.f);
		float out_anim = math::clamp(((g_ctx.globals.absolute_time() - logs[i].log_time) - showtime) / animation_time, 0.f, FLT_MAX);
...
{
	logs.emplace_front(loginfo_t(g_ctx.globals.absolute_time(), text, color));
	...
	if (g_cfg.misc.log_output[EVENTLOG_OUTPUT_CONSOLE])
	{
		...
		m_cvar()->ConsoleColorPrintf(color, crypt_str("[ naval ] ")); //-V807
	...
	if (g_cfg.misc.log_output[EVENTLOG_OUTPUT_CHAT])
	{
		...
		if (!chat)
			chat = (CHudChat*)util::FindHudElement(crypt_str("CHudChat"));

		auto log = crypt_str("[ \fnaval \001] ") + text;
