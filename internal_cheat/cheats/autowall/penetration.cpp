...
void UTIL_ClipTraceToPlayers(const Vector& start, const Vector& end, uint32_t mask, ITraceFilter* filter, CGameTrace* tr, float range) {
	_asm
	{
		...
		lea edx, end
		lea ecx, start
		call g_ctx.addresses.clip_trace_to_players
		add esp, 0xC
	}
}
...
float penetration::scale(player_t* player, float damage, float armor_ratio, int hitgroup) {
	...
	head_scale = player->m_iTeamNum() == 3 ? g_ctx.convars.mp_damage_scale_ct_head->GetFloat() : g_ctx.convars.mp_damage_scale_t_head->GetFloat();
	body_scale = player->m_iTeamNum() == 3 ? g_ctx.convars.mp_damage_scale_ct_body->GetFloat() : g_ctx.convars.mp_damage_scale_t_body->GetFloat();
...
bool penetration::TraceToExit(const Vector& start, const Vector& dir, Vector& out, CGameTrace* enter_trace, CGameTrace* exit_trace) {
	...
	while (dist <= 90.f) {
		...
		if (g_ctx.convars.sv_clip_penetration_traces_to_players->GetInt())
