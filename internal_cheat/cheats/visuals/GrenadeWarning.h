...
public:
	struct data_t {
		...
		data_t(player_t* owner, int index, const Vector& origin, const Vector& velocity, float throw_time, int offset) : data_t() {
			m_owner = owner;
			m_index = index;
		...
		bool physics_simulate() {
			...
			const auto new_velocity_z = m_velocity.z - (g_ctx.convars.sv_gravity->GetFloat() * 0.4f) * m_globals()->m_intervalpertick;
		...
		void physics_trace_entity(const Vector& src, const Vector& dst, std::uint32_t mask, trace_t& trace) {
			...
			m_trace()->TraceLine(src, dst, mask, m_owner, m_collision_group, &trace);
		}
		...
		void physics_push_entity(const Vector& push, trace_t& trace) {
			...
			if (m_index != WEAPON_TAGRENADE
				&& trace.plane.normal.z < std::cos(DEG2RAD(g_ctx.convars.weapon_molotov_maxdetonateslope->GetFloat())))
				return;
		...
		void perform_fly_collision_resolution(trace_t& trace) {
			...
			if (trace.hit_entity) {
				if (util::is_breakable_entity(reinterpret_cast<entity_t*>(trace.hit_entity))) {
				BREAKTROUGH:
					m_velocity *= 0.4f;
		...
		void predict(const Vector& origin, const Vector& velocity, float throw_time, int offset) {
			...
			case WEAPON_MOLOTOV:
			case WEAPON_INCGRENADE:
				m_detonate_time = g_ctx.convars.molotov_throw_detonate_time->GetFloat();
				m_next_think_tick = TIME_TO_TICKS(0.02f);
...
public:
	...
	void do_warning(projectile_t* entity);
};