...
void c_dormant_esp::setup_adjust(player_t* player, SndInfo_t& sound)
{
	...
	m_trace()->TraceRay(Ray_t(src3D, dst3D), MASK_PLAYERSOLID, &filter, &tr);
...
void c_dormant_esp::reset_data()
{
	this->m_utlCurSoundList.RemoveAll();
	this->m_utlvecSoundBuffer.RemoveAll();

	std::memset(this->m_cSoundPlayers, 0, sizeof(this->m_cSoundPlayers));
}