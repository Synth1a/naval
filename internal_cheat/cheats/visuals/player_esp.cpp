...
void playeresp::paint_traverse()
{
	...
	auto radar_base = (DWORD)util::FindHudElement(crypt_str("CCSGO_HudRadar"));
	...
	for (auto i = 1; i <= m_globals()->m_maxclients; ++i) //-V807
	{
		...
		if (g_cfg.player.type[type].skeleton)
		{
			...
			draw_skeleton(e, Color::Yellow, e->m_CachedBoneData().Base());

			const auto ideal = lagcompensation::get().GetLatestRecord(e);
			if (ideal.has_value())
			{
				const auto record = ideal.value();

				draw_skeleton(e, Color::White, record->m_pMatrix.main);
				draw_skeleton(e, Color::Red, record->m_pMatrix.center);
				draw_skeleton(e, Color::Green, record->m_pMatrix.left);
				draw_skeleton(e, Color::Blue, record->m_pMatrix.right);
			}
		}
		...
		if (type == ENEMY || type == TEAM)
		{
			...
			if (type == ENEMY)
			{
				if (g_cfg.player.arrows && g_ctx.local()->is_alive())
				{
					...
					if (e->IsDormant())
						color = Color(130, 130, 130, (int)(255.0f * esp_alpha_fade[i] * 0.4f));
...
void playeresp::draw_health(player_t* m_entity, const Box& box, const HPInfo& hpbox)
{
	...
	auto color = m_entity->IsDormant() ? Color(130, 130, 130) : Color(static_cast<int> ((130.0f - hpbox.hp * 1.3f)), static_cast<int> ((hpbox.hp * 2.55f)), 10);
...
bool playeresp::draw_ammobar(player_t* m_entity, const Box& box)
{
	...
	auto animlayer = m_entity->get_animlayers()[ANIMATION_LAYER_WEAPON_ACTION];
...
void playeresp::draw_name(player_t* m_entity, const Box& box)
{
	...
	if (m_engine()->GetPlayerInfo(m_entity->EntIndex(), &player_info))
	{
		...
		render::get().text(fonts[NAME], box.x + (box.w / 2), box.y - 14, color, HFONT_CENTERED_X, name.c_str());
	}
}
...
void playeresp::draw_weapon(player_t* m_entity, const Box& box, bool space)
{
	...
	if (g_cfg.player.type[type].weapon[WEAPON_ICON])
		render::get().text(fonts[WEAPON_ICON_FONT], box.x + box.w / 2, pos, color, HFONT_CENTERED_X, weapon->get_icon());
}
...
void playeresp::draw_flags(player_t* e, const Box& box)
{
	...
	if (g_cfg.player.type[type].flags[FLAGS_MONEY])
	{
		...
		color.SetAlpha(255.0f * esp_alpha_fade[e->EntIndex()] / 1.3);

		render::get().text(fonts[SMALL_ESP], _x, _y, color, HFONT_CENTERED_NONE, crypt_str("%i$"), e->m_iAccount());
		_y += 8;
	}
	...
	if (g_cfg.player.type[type].flags[FLAGS_ARMOR])
	{
		if (e->m_ArmorValue() > 0) {
			...
			if (e->m_bHasHelmet()) {
				render::get().text(fonts[SMALL_ESP], _x, _y, color, HFONT_CENTERED_NONE, crypt_str("HK"));
				_y += 8;
			}
			else {
				render::get().text(fonts[SMALL_ESP], _x, _y, color, HFONT_CENTERED_NONE, crypt_str("K"));
				_y += 8;
			}
		}
	}
	...
	if (g_cfg.player.type[type].flags[FLAGS_KIT] && e->m_bHasDefuser())
	{
		...
		render::get().text(fonts[SMALL_ESP], _x, _y, color, HFONT_CENTERED_NONE, crypt_str("KIT"));
		_y += 8;
	}
	...
	if (g_cfg.player.type[type].flags[FLAGS_SCOPED])
	{
		...
		if (scoped)
		{
			...
			render::get().text(fonts[SMALL_ESP], _x, _y, color, HFONT_CENTERED_NONE, crypt_str("ZOOM"));
			_y += 8;
		}
	}
	...
	if (g_cfg.player.type[type].flags[FLAGS_FAKEDUCKING])
	{
		...
		if (animstate)
		{
			...
			if (fakeducking() && e->m_fFlags() & FL_ONGROUND && !animstate->m_bLanding)
			{
				...
				render::get().text(fonts[SMALL_ESP], _x, _y, color, HFONT_CENTERED_NONE, crypt_str("FD"));
				_y += 8;
			}
		}
	}

	if (g_cfg.player.type[type].flags[FLAGS_VULNERABLE])
	{
		auto activity = e->sequence_activity(e->get_animlayers()[1].m_nSequence);
		auto reloading = activity == ACT_CSGO_RELOAD && e->get_animlayers()[1].m_flWeight;

		if (weapon->is_non_aim() || reloading) {
			auto color = e->IsDormant() ? Color(130, 130, 130, 130) : Color(205, 180, 110);
			color.SetAlpha(255.0f * esp_alpha_fade[e->EntIndex()] / 1.3);

			render::get().text(fonts[SMALL_ESP], _x, _y, color, HFONT_CENTERED_NONE, crypt_str("VULNERABLE"));
			_y += 8;
		}
	}
	...
	if (g_cfg.player.type[type].flags[FLAGS_PING])
	{
		if (e->is_bot())
		{
			...
			render::get().text(fonts[SMALL_ESP], _x, _y, color, HFONT_CENTERED_NONE, crypt_str("BOT"));
			_y += 8;
		}
		else
		{
			auto latency = math::clamp(m_playerresource()->GetPing(e->EntIndex()), 0, 999);
			std::string delay = std::to_string(latency) + crypt_str("MS");
			...
			render::get().text(fonts[SMALL_ESP], _x, _y, color, HFONT_CENTERED_NONE, delay.c_str());
			_y += 8;
		}
	}

	if (g_cfg.player.type[type].flags[FLAGS_C4] && e->has_c4())
	{
		auto color = e->IsDormant() ? Color(130, 130, 130, 130) : Color(163, 49, 93);
		color.SetAlpha(255.0f * esp_alpha_fade[e->EntIndex()] / 1.3);

		render::get().text(fonts[SMALL_ESP], _x, _y, color, HFONT_CENTERED_NONE, crypt_str("C4"));
		_y += 8;
	}
}
...
void playeresp::draw_lines(player_t* e)
{
	...
	if (!math::world_to_screen(e->m_vecOrigin(), angle))
		return;
	...
	render::get().line(g_ctx.globals.screen_width / 2, g_ctx.globals.screen_height, angle.x, angle.y, color);
}

void playeresp::reset_data()
{
	std::memset(this->esp_alpha_fade, 0, sizeof(this->esp_alpha_fade));
	std::memset(this->health, 0, sizeof(this->health));
	std::memset(this->hp_info, 0, sizeof(this->hp_info));
}