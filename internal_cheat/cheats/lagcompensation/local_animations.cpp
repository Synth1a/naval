...
void local_animations::do_animation_event(Local_data::Anim_data* data)
{
	...
	if (this->m_movetype != MOVETYPE_LADDER && g_ctx.local()->get_move_type() == MOVETYPE_LADDER)
		g_ctx.local()->get_animation_state()->set_layer_sequence(pLandOrClimbLayer, ACT_CSGO_CLIMB_LADDER);
	else if (this->m_movetype == MOVETYPE_LADDER && g_ctx.local()->get_move_type() != MOVETYPE_LADDER)
		g_ctx.local()->get_animation_state()->set_layer_sequence(pJumpOrFallLayer, ACT_CSGO_FALL);
	else
	{
		if (g_ctx.local()->m_fFlags() & FL_ONGROUND)
		{
			if (!(this->m_flags & FL_ONGROUND))
				g_ctx.local()->get_animation_state()->set_layer_sequence(pLandOrClimbLayer, g_ctx.local()->get_animation_state()->m_flDurationInAir > 1.0f ? ACT_CSGO_LAND_HEAVY : ACT_CSGO_LAND_LIGHT);
		}
		else if (this->m_flags & FL_ONGROUND)
		{
			if (g_ctx.local()->m_vecVelocity().z > 0.0f)
				g_ctx.local()->get_animation_state()->set_layer_sequence(pJumpOrFallLayer, ACT_CSGO_JUMP);
			else
				g_ctx.local()->get_animation_state()->set_layer_sequence(pJumpOrFallLayer, ACT_CSGO_FALL);
		}
	}
	...
	this->m_flags = engineprediction::get().get_netvars(data->anim_cmd->m_command_number).m_fFlags;
}
...
void local_animations::update_fake_animations(Local_data::Anim_data* data)
{
	...
	if (allocate)
	{
		auto* state = reinterpret_cast<c_baseplayeranimationstate*>(m_memalloc()->Alloc(sizeof(c_baseplayeranimationstate)));

		if (state)
			util::create_state(state, g_ctx.local());
	...
	if (g_ctx.globals.m_ragebot_shot_nr == data->anim_cmd->m_command_number)
	{
	...
	if (data->send_packet)
	{
		...
		util::update_state(this->local_data.fake_animstate, target_angle);

		// now we setup the bones after everything is done.
		g_ctx.local()->setup_bones(g_ctx.animations.local.m_fake_matrix);
		...
		if (!this->local_data.visualize_lag)
		{
			...
			for (auto& i : g_ctx.animations.local.m_fake_matrix)
			{
...
void local_animations::update_local_animations(Local_data::Anim_data* data)
{
	...
	g_ctx.local()->m_fFlags() = engineprediction::get().get_netvars(data->anim_cmd->m_command_number).m_fFlags;

	// if our spawntime didn't match our entity spawntime or the entity of the animstate holder is not us?.
	if (g_ctx.local()->m_flSpawnTime() != this->m_real_spawntime || g_ctx.local()->get_animation_state()->m_pBasePlayer != g_ctx.local())
	{
	...
	g_ctx.local()->get_animation_state()->m_flPrimaryCycle = this->m_real_layers[ANIMATION_LAYER_MOVEMENT_MOVE].m_flCycle;
	g_ctx.local()->get_animation_state()->m_flMoveWeight = this->m_real_layers[ANIMATION_LAYER_MOVEMENT_MOVE].m_flWeight;
	g_ctx.local()->get_animation_state()->m_nStrafeSequence = this->m_real_layers[ANIMATION_LAYER_MOVEMENT_STRAFECHANGE].m_nSequence;
	g_ctx.local()->get_animation_state()->m_flStrafeChangeCycle = this->m_real_layers[ANIMATION_LAYER_MOVEMENT_STRAFECHANGE].m_flCycle;
	g_ctx.local()->get_animation_state()->m_flStrafeChangeWeight = this->m_real_layers[ANIMATION_LAYER_MOVEMENT_STRAFECHANGE].m_flWeight;
	g_ctx.local()->get_animation_state()->m_flAccelerationWeight = this->m_real_layers[ANIMATION_LAYER_LEAN].m_flWeight;
	...
	g_ctx.local()->m_thirdPersonViewAngles() = (((exploitsystem::get().hide_shots_key) && !g_ctx.globals.fakeducking && lock_viewangles) ? non_shot_target_angle : target_angle);

	// allows re-animating at the same tick.
	if (g_ctx.local()->get_animation_state()->m_nLastUpdateFrame == m_globals()->m_framecount)
		g_ctx.local()->get_animation_state()->m_nLastUpdateFrame = m_globals()->m_framecount - 1;

	// same as above.
	if (g_ctx.local()->get_animation_state()->m_flLastUpdateTime == m_globals()->m_curtime)
		g_ctx.local()->get_animation_state()->m_flLastUpdateTime = m_globals()->m_curtime - m_globals()->m_intervalpertick;
	...
	if (data->send_packet) 
	{
		...
		// save off real data.
		this->m_real_angles = g_ctx.local()->get_animation_state()->m_flFootYaw;

		// rebuilt bones and interpolate our origin with our matrix origin..
		g_ctx.animations.local.m_real_matrix_ret = g_ctx.local()->setup_bones(g_ctx.animations.local.m_real_matrix);

		g_ctx.local()->set_animlayers(this->get_animlayers().data());
		g_ctx.local()->set_poseparameter(this->m_real_poses);

		for (int i = NULL; i < MAXSTUDIOBONES; i++)
			g_ctx.globals.local_origin[i] = g_ctx.local()->GetAbsOrigin() - g_ctx.animations.local.m_real_matrix[i].GetOrigin();
...
void local_animations::setup_shoot_position(CUserCmd* m_pcmd)
{
	...
	g_ctx.local()->setup_bones(NULL, true);
...
void local_animations::on_update_clientside_animation(player_t* player)
{
	...
	for (int i = NULL; i < MAXSTUDIOBONES; i++)
		g_ctx.animations.local.m_real_matrix[i].SetOrigin(player->GetAbsOrigin() - g_ctx.globals.local_origin[i]);

	// set our cached bone to our matrix.
	std::memcpy(player->m_CachedBoneData().Base(), g_ctx.animations.local.m_real_matrix, player->m_CachedBoneData().Count() * sizeof(matrix3x4_t));

	// set our accessor bones to our matrix.
	player->m_BoneAccessor()->m_pBones = g_ctx.animations.local.m_real_matrix;
...
{
	if (g_ctx.animations.local.m_real_matrix_ret)
		std::memcpy(matrix, g_ctx.animations.local.m_real_matrix, player->m_CachedBoneData().Count() * sizeof(matrix3x4_t));
}

void local_animations::reset_data()
{
	std::memset(g_ctx.globals.local_origin, 0, sizeof(g_ctx.globals.local_origin));
	std::memset(g_ctx.animations.local.m_fake_matrix, 0, sizeof(g_ctx.animations.local.m_fake_matrix));
	std::memset(g_ctx.animations.local.m_real_matrix, 0, sizeof(g_ctx.animations.local.m_real_matrix));
	...
	g_ctx.animations.local.m_real_matrix_ret = false;
