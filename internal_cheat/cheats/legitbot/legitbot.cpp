...
void legit_bot::do_smooth(const Vector& currentAngle, const Vector& aimAngle, Vector& angle)
{
	...
	static auto recoil_scale = g_ctx.convars.weapon_recoil_scale->GetFloat();
...
{
	static auto LineGoesThroughSmokeFn = (bool(*)(Vector vStartPos, Vector vEndPos))g_ctx.addresses.line_goes_through_smoke;
...
void legit_bot::createmove(CUserCmd* cmd)
{
	...
	if (target_switch_delay > m_globals()->m_realtime)
	{
		...
		if ((g_cfg.keybinds.key[LEGIT_BOT_KEYBIND].active || g_cfg.keybinds.key[AUTO_FIRE_KEYBIND].active) && !g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].rcs_type)
			do_rcs(cmd);
	...
	// legitbot key check
	if (!g_cfg.keybinds.key[LEGIT_BOT_KEYBIND].active && !g_cfg.keybinds.key[AUTO_FIRE_KEYBIND].active)
	{
		autofire_delay = 0;
	...
	if (!g_ctx.local()->m_iShotsFired() && g_cfg.keybinds.key[AUTO_FIRE_KEYBIND].active && autofire_delay < g_cfg.legitbot.autofire_delay)
	{
		autofire_delay += 1;
		return;
	}
	...
	if (g_cfg.keybinds.key[AUTO_FIRE_KEYBIND].active && g_ctx.globals.weapon->can_fire(true) && hitchanced)
		cmd->m_buttons |= IN_ATTACK;
...
void legit_bot::find_best_point(CUserCmd* cmd, float fov_v)
{
	...
	if (g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].priority == 0)
	{
		for (auto bone : { HITBOX_HEAD, HITBOX_PELVIS, HITBOX_CHEST, HITBOX_UPPER_CHEST, HITBOX_STOMACH })
		{
			...
			in.m_damage = 1.f;
			in.m_damage_pen = 1.f;
			in.m_can_pen = g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].awall_dmg;
			in.m_target = target;
			...
			if (penetration::get().run(&in, &out)) {
				if (!out.m_visible && !g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].awall_dmg)
					continue;

				if (!out.m_visible && g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].awall_dmg > 0 && out.m_damage < g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].awall_dmg)
					continue;
...
int legit_bot::hitchance(player_t* target, const Vector& aim_angle, const Vector& point, int hitbox) //-V688
{
	...
	if (g_ctx.globals.weapon->get_inaccuracy() - 0.000001f < inaccuracy)
		final_hitchance = 101;
	else
	{
		...
		for (auto i = 0; i < 256; ++i)
		{
			auto inaccuracy = spread_values[i][0] * g_ctx.globals.weapon->get_inaccuracy();
			auto spread = spread_values[i][1] * g_ctx.globals.weapon->get_spread();
...
{
	static auto fn = g_ctx.addresses.clip_ray_to_hitbox;
...
bool legit_bot::hitbox_intersection(player_t* e, matrix3x4_t* matrix, int hitbox, const Vector& start, const Vector& end)
{
	...
	return clip_ray_to_hitbox(Ray_t(start, end), studio_hitbox, matrix[studio_hitbox->bone], trace) >= 0;
}