...
void render::gradient(int x, int y, int w, int h, Color first, Color second, GradientType type) 
{
	...
	auto filled_rect_fade = [&](bool reversed, float alpha) {
		...
		call_virtual< Fn >(m_surface(), g_ctx.indexes.at(9)) (
...
void render::polygon_outline(int count, std::array< Vector2D, 3 >points, Color colorLine)
{
	static int x[128];
	static int y[128];
	...
	m_surface()->DrawSetColor(colorLine);
	m_surface()->DrawPolyLine(x, y, count);
}
...
void render::add_textured_polygon(int n, Vertex_t* vertice, Color colour) {
	colour.SetAlpha(static_cast<int>(colour.a() * alpha_factor));

	static int texture_id = m_surface()->CreateNewTextureID(true);

	m_surface()->DrawSetColor(colour);
	m_surface()->DrawSetTexture(texture_id);
	m_surface()->DrawTexturedPolygon(n, vertice);
}
...
void render::draw_filled_triangle(std::array< Vector2D, 3 >points, Color colour) {
	...
	add_textured_polygon(3, verts, colour);
}

void render::line(int x, int y, int x2, int y2, Color color, int thickline) 
{
	if (!m_surface())
		return;
	...
	if (thickline) {
		/*
		Vertex_t vertices[6];
		vertices[0].Init(Vector2D(x, y));
		vertices[1].Init(Vector2D(x2, y2));
		vertices[2].Init(Vector2D(x, y + thickline));
		vertices[3].Init(Vector2D(x2, y2 + thickline));
		vertices[4].Init(Vector2D(x, y - thickline));
		vertices[5].Init(Vector2D(x2, y2 - thickline));
...
void render::filled_rect_world(Vector camera, const trace_t& tr, Color color, int size)
{
	Vector forward = Vector(size, size, size);
	Vector right = math::cross_product(forward, tr.plane.normal);
	Vector up = math::cross_product(tr.plane.normal, right);

	Vector a, b, c, d;
	if (math::world_to_screen(tr.endpos + up, a) && math::world_to_screen(tr.endpos - up, b) &&
		math::world_to_screen(tr.endpos + right, c) && math::world_to_screen(tr.endpos - right, d))
	{
		Vertex_t vertices[4];
		static int tex = m_surface()->CreateNewTextureID(true);

		m_surface()->DrawSetTexture(tex);
		m_surface()->DrawSetColor(color);
		...
		m_surface()->DrawSetColor(color.r(), color.g(), color.b(), 255);
		m_surface()->DrawPolyLine(x, y, 4);
	}
}
...
void render::ColorConvertHSVtoRGB(float h, float s, float v, float& out_r, float& out_g, float& out_b)
{
	if (!s) //-V550
	{
		out_r = out_g = out_b = v;
...
void render::Draw3DCircle(const Vector& origin, float radius, Color color, int thickline)
{
	...
	for (auto rotation = 0.0f; rotation <= M_PI * 2.0f; rotation += step) //-V1034
	{
		...
		m_trace()->TraceRay(Ray_t(origin, pos), MASK_SHOT_BRUSHONLY, &filter, &trace);

		if (math::world_to_screen(trace.endpos, screenPos))
		{
			if (!prevScreenPos.IsZero()) {
				if (thickline)
					line(prevScreenPos.x, prevScreenPos.y, screenPos.x, screenPos.y, color, 1);
				else
...
void render::Draw3DFilledCircle(const Vector& origin, float radius, Color color, int thickline)
{
	auto prevScreenPos = ZERO; //-V656
	auto step = M_PI * 2.0f / 72.0f;
	...
	for (auto rotation = 0.0f; rotation <= M_PI * 2.0f; rotation += step) //-V1034
	{
		...
		if (math::world_to_screen(pos, screenPos))
		{
			if (!prevScreenPos.IsZero() && prevScreenPos.IsValid() && screenPos.IsValid() && prevScreenPos != screenPos)
			{
				...
				{
					Vector2D(screen.x, screen.y),
					Vector2D(screenPos.x, screenPos.y),
					Vector2D(prevScreenPos.x, prevScreenPos.y)
				};

				draw_filled_triangle(points, Color(color.r(), color.g(), color.b(), color.a()));
			}
...
void render::Draw3DRainbowCircle(const Vector& origin, float radius, int thickline)
{
	...
	for (auto rotation = 0.0f; rotation <= M_PI * 2.0f; rotation += step) //-V1034
	{
		...
		if (math::world_to_screen(trace.endpos, screenPos))
		{
			if (!prevScreenPos.IsZero())
			{
				...
				line(prevScreenPos.x, prevScreenPos.y, screenPos.x, screenPos.y, Color(r, g, b));
			}
...
RECT render::viewport()
{
	...
	viewport.right = g_ctx.globals.screen_width;
	viewport.bottom = g_ctx.globals.screen_height;
