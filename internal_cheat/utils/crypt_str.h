...
inline char const* ConvConstChar(std::string str) {
    char const* c = strcpy((char*)malloc(str.length() + 1), str.c_str());
    return c;
}
...
#define crypt_str(str) xorstr(str).crypt_get()
#define crypt_arr(str) ConvConstChar(crypt_str(str))
