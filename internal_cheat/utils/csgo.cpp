...
IDirect3DDevice9* C_CSGO::m_device() {
	if (!p_device)
		p_device = **reinterpret_cast<IDirect3DDevice9***>(g_ctx.addresses.utility.direct3ddevice9);
...
IClientLeafSystem* C_CSGO::m_leafsystem() {
	if (!p_leafsystem)
		p_leafsystem = get_interface<IClientLeafSystem>(crypt_str("client.dll"), crypt_str("ClientLeafSystem"));
...
IViewRenderBeams* C_CSGO::m_viewrenderbeams() {
	if (!p_viewrenderbeams)
		p_viewrenderbeams = *reinterpret_cast<IViewRenderBeams**>(g_ctx.addresses.utility.viewrenderbeams);
...
CGlowObjectManager* C_CSGO::m_glow() {
	if (!p_glow)
		p_glow = *reinterpret_cast<CGlowObjectManager**>(g_ctx.addresses.utility.glowobjectmanager);
...
IMoveHelper* C_CSGO::m_movehelper() {
	if (!p_movehelper)
		p_movehelper = **reinterpret_cast<IMoveHelper***>(g_ctx.addresses.utility.movehelper);
...
CInput* C_CSGO::m_input() {
	if (!p_input)
		p_input = *reinterpret_cast<CInput**>(g_ctx.addresses.utility.input);
...
C_CSPlayerResource* C_CSGO::m_playerresource() {
	if (!p_playerresource)
		p_playerresource = **reinterpret_cast<C_CSPlayerResource***>(g_ctx.addresses.utility.playerresource);
...
CSGameRulesProxy* C_CSGO::m_gamerules() {
	if (!p_gamerules)
		p_gamerules = **reinterpret_cast<CSGameRulesProxy***>(g_ctx.addresses.utility.gamerules);
...
IBaseFileSystem* C_CSGO::m_basefilesys() {
	if (!p_basefilesys)
		p_basefilesys = get_interface<IBaseFileSystem>(crypt_str("filesystem_stdio.dll"), crypt_str("VBaseFileSystem"));
...
DWORD C_CSGO::m_postprocessing() {
	if (!p_postprocessing)
		p_postprocessing = *(DWORD*)(g_ctx.addresses.utility.postprocessing);
...
DWORD C_CSGO::m_ccsplayerrenderablevftable() {
	if (!p_ccsplayerrenderablevftable)
		p_ccsplayerrenderablevftable = *(DWORD*)(g_ctx.addresses.utility.ccsplayerrenderablevftable);
