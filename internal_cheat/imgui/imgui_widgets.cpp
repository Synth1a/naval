...
bool ImGui::Tab(const char* label, const char* icon, bool selected, bool is_for_visual)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = ImGui::CalcItemSize(ImVec2(32, 27), label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
    ...
    const ImVec4 text_dis = ImVec4(0.27f, 0.27f, 0.27f, 1.f);
    const ImVec4 text_act = ImVec4(0.85f, 0.87f, 0.91f, 1.f);

    float deltatime = 1.5f * ImGui::GetIO().DeltaTime;

    static std::map<ImGuiID, ImVec4> text_animation;
    auto it_icon = text_animation.find(id);
    if (it_icon == text_animation.end())
    {
        text_animation.insert({ id, text_dis });
        it_icon = text_animation.find(id);
    }
    ...
    if (selected)
    {
        window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(ImVec4(0.27f, 0.27f, 0.37f, c_menu::get().public_alpha)), 5);
        window->DrawList->AddCircleFilled(ImVec2(bb.Max.x - 17, bb.Min.y + (size.y / 2) + 1), 10, ImColor(ImVec4(0.40, 0.42, 0.45, c_menu::get().public_alpha)), 360);
    }

    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(it_icon->second.x, it_icon->second.y, it_icon->second.z, c_menu::get().public_alpha));

    ImGui::PushFont(g_ctx.fonts.tab_icon);
    ...
    ImGui::RenderText(ImVec2(bb.Min.x + add, bb.Min.y + (size.y / 2) - (icon_size.y / 2) - 2), icon);
...
void ImGui::Keybind(const char* label, key_bind* key_bind, const char* unique_id)
{
    ...
    auto textsize = CalcTextSize(text.c_str()).x + 8;
    auto labelsize = CalcTextSize(label);

    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, c_menu::get().public_alpha));
    ImGui::Text(label);
    ImGui::PopStyleColor();
    ImGui::SameLine();

    ImGui::SetCursorPosX(GetWindowSize().x - (GetWindowSize().x - CalcItemWidth()) - max(50, textsize));
    ImGui::SetCursorPosY(GetCursorPosY() - 3);

    if (ImGui::KeybindButton(text.c_str(), unique_id, key_bind, ImVec2(max(50, textsize), 23), clicked))
        clicked = true;
    ...
    static std::map<const char*, float> open_animation;
    auto it_open = open_animation.find(unique_id);
    if (it_open == open_animation.end())
    {
        open_animation.insert({ unique_id, 0.f });
        it_open = open_animation.find(unique_id);
    }
    it_open->second = ImClamp(it_open->second + (10.f * GetIO().DeltaTime * ((clicked || ImGui::IsPopupOpen(unique_id)) ? 1.f : -1.f)), 0.01f, 1.f);

    if (ImGui::BeginPopup(unique_id, ImGuiWindowFlags_NoBackground))
    {
        ImGuiWindow* window = ImGui::GetCurrentWindow();
        ImRect bb = window->Rect();

        window->DrawList->AddRectFilled(ImVec2(bb.Min.x + 6, bb.Min.y + 6), ImVec2(bb.Max.x - 6, bb.Max.y - 6), ImColor(ImVec4(0.04f, 0.04f, 0.04f, it_open->second)), 2);

        if (ImGui::Selectable(crypt_str("Hold"), &hold))
        {
            if (hold)
            {
                toggle = false;
        ...
        if (ImGui::Selectable(crypt_str("Toggle"), &toggle))
        {
            if (toggle)
            {
                hold = false;
...
bool ImGui::KeybindButton(const char* label, const char* unique_id, key_bind* key_bind, const ImVec2& size_arg, bool clicked, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImVec4 hover_act = ImVec4(1.f, 1.f, 1.f, 1.f);
    ...
    window->DrawList->AddRectFilled(bb.Min, bb.Max - ImVec2(0, 5), ImColor(ImVec4(0.04f, 0.04f, 0.04f, c_menu::get().public_alpha)), 2.f);
    window->DrawList->AddRect(bb.Min, bb.Max - ImVec2(0, 5), ImColor(ImVec4(0.11f, 0.11f, 0.11f, c_menu::get().public_alpha)), 2.f);
    window->DrawList->AddText(bb.Min + ImVec2(size_arg.x / 2 - CalcTextSize(label).x / 2, size_arg.y / 2 - CalcTextSize(label).y / 2 - 3), ImColor(ImVec4(it_hcolor->second.x, it_hcolor->second.y, it_hcolor->second.z, c_menu::get().public_alpha)), label);
...
bool ImGui::CustomButton(const char* label, const char* label_id, const ImVec2& size_arg, bool use_icon, ImFont* icon_font, const char* icon_label)
{
    ...
    const float sizeaa = ImGui::GetWindowWidth() - 20;

    const ImRect size2(window->DC.CursorPos, window->DC.CursorPos + ImVec2(sizeaa, 24));
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(sizeaa, 26));
    ItemSize(size2, style.FramePadding.y);
    if (!ItemAdd(bb, id))
        return false;
    ...
    it_hover->second = math::clamp(it_hover->second + (3.f * ImGui::GetIO().DeltaTime * (hovered ? c_menu::get().public_alpha : -1.f)), 0.0f, 1.f);
    ...
    if (strcmp("Save configuration", label) != 0 && strcmp("Remove configuration", label) != 0 && strcmp("Save & Close", label) != 0)
    {
        if (pressed || it_circle->second.active)
        {
            ...
            it_circle->second.alpha = math::clamp(it_circle->second.alpha - (2.f * ImGui::GetIO().DeltaTime), 0.0f, (c_menu::get().public_alpha - 0.2f));
    ...
    const ImVec4 text_act = ImVec4(1.f, 1.f, 1.f, c_menu::get().public_alpha);
    const ImVec4 icon_act = ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, c_menu::get().public_alpha);

    window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(ImVec4(0.04f, 0.04f, 0.04f, c_menu::get().public_alpha)), 3.f);
    ...
    ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, it_hover->second)), 2.f);
...
void ImGui::Scrollbar(ImGuiAxis axis)
{
    ...
    ImDrawCornerFlags rounding_corners = ImDrawCornerFlags_None;
    if (axis == ImGuiAxis_X)
    {
    ...
    auto scroll = window->Scroll[axis];
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ...
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, ImColor(ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, c_menu::get().public_alpha)), style.ScrollbarRounding);
...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec2& padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ...
    if (hovered)
    {
        auto to_outline_alpha = c_menu::get().public_alpha;
        if (it_anim->second != to_outline_alpha)
        {
            if (it_anim->second < to_outline_alpha)
                it_anim->second = ImMin(it_anim->second + deltatime, to_outline_alpha);
            else if (it_anim->second > to_outline_alpha)
                it_anim->second = ImMax(to_outline_alpha, it_anim->second - deltatime);
        }
    }
    else
    {
        auto to_outline_alpha = 0.f;
        if (it_anim->second != to_outline_alpha)
        {
            if (it_anim->second < to_outline_alpha)
                it_anim->second = ImMin(it_anim->second + deltatime, to_outline_alpha);
    ...
    window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(ImVec4(0.04f, 0.04f, 0.04f, c_menu::get().public_alpha)), 3.f);
    window->DrawList->AddRect(bb.Min, bb.Max, ImColor(ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, 0.05f + it_anim->second)), 3.f);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    float deltatime = 3.f * ImGui::GetIO().DeltaTime;

    static std::map<ImGuiID, checkbox_animation> animation;
    auto it_animation = animation.find(id);
    if (it_animation == animation.end())
    {
        animation.insert({ id, {0.f, 0.f, 0.f} });
        it_animation = animation.find(id);
    }

    if (hovered && !*v)
    {
        auto to_rect_alpha = c_menu::get().public_alpha;
       
        if (it_animation->second.hovered_rect_alpha != to_rect_alpha)
        {
            if (it_animation->second.hovered_rect_alpha < to_rect_alpha)
                it_animation->second.hovered_rect_alpha = ImMin(it_animation->second.hovered_rect_alpha + deltatime, to_rect_alpha);
            else if (it_animation->second.hovered_rect_alpha > to_rect_alpha)
                it_animation->second.hovered_rect_alpha = ImMax(to_rect_alpha, it_animation->second.hovered_rect_alpha - deltatime);
        }
    }
    else
    {
        auto to_rect_alpha = 0.f;
    ...
    {
        auto to_rect_alpha = c_menu::get().public_alpha;
        auto to_active_text = 0.60f;

        if (it_animation->second.active_rect_alpha != to_rect_alpha)
        {
            if (it_animation->second.active_rect_alpha < to_rect_alpha)
                it_animation->second.active_rect_alpha = ImMin(it_animation->second.active_rect_alpha + deltatime, to_rect_alpha);
            else if (it_animation->second.active_rect_alpha > to_rect_alpha)
                it_animation->second.active_rect_alpha = ImMax(to_rect_alpha, it_animation->second.active_rect_alpha - deltatime);
        }

        if (it_animation->second.active_text_anim != to_active_text)
        {
            if (it_animation->second.active_text_anim < to_active_text)
                it_animation->second.active_text_anim = ImMin(it_animation->second.active_text_anim + deltatime, to_active_text);
            else if (it_animation->second.active_text_anim > to_active_text)
                it_animation->second.active_text_anim = ImMax(to_active_text, it_animation->second.active_text_anim - deltatime);
        }
    }
    else
    {
        auto disable_num = 0.f;

        if (it_animation->second.active_rect_alpha != disable_num)
        {
            if (it_animation->second.active_rect_alpha < disable_num)
                it_animation->second.active_rect_alpha = ImMin(it_animation->second.active_rect_alpha + deltatime, disable_num);
            else if (it_animation->second.active_rect_alpha > disable_num)
                it_animation->second.active_rect_alpha = ImMax(disable_num, it_animation->second.active_rect_alpha - deltatime);
        }

        if (it_animation->second.active_text_anim != disable_num)
        {
            if (it_animation->second.active_text_anim < disable_num)
                it_animation->second.active_text_anim = ImMin(it_animation->second.active_text_anim + deltatime, disable_num);
            else if (it_animation->second.active_text_anim > disable_num)
                it_animation->second.active_text_anim = ImMax(disable_num, it_animation->second.active_text_anim - deltatime);
        }
    }

    /* Draw background for the checkmark. */
    window->DrawList->AddRectFilled(rect.Max - ImVec2(15, 15), rect.Max, ImColor(ImVec4(0.04f, 0.04f, 0.04f, c_menu::get().public_alpha)), 2.f);
    
    /* Hovered stuff. */
    window->DrawList->AddRect(rect.Max - ImVec2(15, 15), rect.Max, ImColor(ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, it_animation->second.hovered_rect_alpha)), 2.f);

    /* Draw the background for when it's active. */
    window->DrawList->AddRectFilled(rect.Max - ImVec2(14, 14), rect.Max - ImVec2(1, 1), ImColor(ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, it_animation->second.active_rect_alpha)), 2.f);

    /* Render the checkmark, ez.*/
    RenderCheckMark(window->DrawList, rect.Max - ImVec2(12, 13), ImColor(ImVec4(1.f, 1.f, 1.f, it_animation->second.active_rect_alpha)), 9.f);

    /* Draw the functions name. */
    window->DrawList->AddText(rect.Min + ImVec2(0, 2), ImColor(ImVec4(1.f, 1.f, 1.f, (c_menu::get().public_alpha - 0.60f + it_animation->second.active_text_anim))), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImGuiID popup_id = ImHashStr(crypt_str("##ComboPopup"), 0, id);
    ...
    {
        auto to_outline_alpha = c_menu::get().public_alpha;
        if (it_anim->second.active_outline_anim != to_outline_alpha)
        {
            if (it_anim->second.active_outline_anim < to_outline_alpha)
                it_anim->second.active_outline_anim = ImMin(it_anim->second.active_outline_anim + deltatime, to_outline_alpha);
            else if (it_anim->second.active_outline_anim > to_outline_alpha)
                it_anim->second.active_outline_anim = ImMax(to_outline_alpha, it_anim->second.active_outline_anim - deltatime);
        }
    }
    else
    {
    ...
    if (popup_open)
    {
        auto to_rect_alpha = 1.f;

        if (it_anim->second.active_circle_anim != to_rect_alpha)
        {
            if (it_anim->second.active_circle_anim < to_rect_alpha)
                it_anim->second.active_circle_anim = ImMin(it_anim->second.active_circle_anim + deltatime, to_rect_alpha);
            else if (it_anim->second.active_circle_anim > to_rect_alpha)
                it_anim->second.active_circle_anim = ImMax(to_rect_alpha, it_anim->second.active_circle_anim - deltatime);
        }

        if (it_anim->second.open_anim != to_rect_alpha)
        {
            if (it_anim->second.open_anim < to_rect_alpha)
                it_anim->second.open_anim = ImMin(it_anim->second.open_anim + deltatime, to_rect_alpha);
            else if (it_anim->second.open_anim > to_rect_alpha)
                it_anim->second.open_anim = ImMax(to_rect_alpha, it_anim->second.open_anim - deltatime);
        }
    }
    else
    {
    ...
    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, ImColor(ImVec4(0.04f, 0.04f, 0.04f, c_menu::get().public_alpha)), 3.f);

    window->DrawList->AddRect(clickable.Min, clickable.Max, ImColor(ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, 0.05f + it_anim->second.active_outline_anim)), 3.f);

    window->DrawList->AddText(rect.Min, ImColor(ImVec4(1.f, 1.f, 1.f, c_menu::get().public_alpha)), label);

    PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, c_menu::get().public_alpha));
    RenderTextClipped(clickable.Min + ImVec2(5, 7), clickable.Max - ImVec2(20, 0), preview_value, NULL, NULL, ImVec2(0.0f, 0.0f));
    PopStyleColor();

    window->DrawList->AddCircleFilled(rect.Max - ImVec2(11, 13), 4.f + it_anim->second.active_circle_anim, ImColor(ImVec4(1.f, 1.f, 1.f, c_menu::get().public_alpha)));

    if (!popup_open)
    {
        it_anim->second.open_anim = 0.f;
        return false;
    }
    ...
    char name[16];
    ImFormatString(name, IM_ARRAYSIZE(name), crypt_str("##Combo_%02d"), g.BeginPopupStack.Size); // Recycle windows based on depth
    ...
    PushStyleColor(ImGuiCol_PopupBg, ImVec4(ImColor(0.04f, 0.04, 0.04f, c_menu::get().public_alpha)));
    bool ret = Begin(name, NULL, window_flags);
    PopStyleVar(3);
    PopStyleColor(1);

    if (it_anim->second.open_anim < 0.99f)
        PopStyleVar();
...
bool ImGui::MultiCombo(std::string name, const char** displayName, std::vector<int>& data, int dataSize) {
    ...
    for (int i = 0; i < dataSize; i++) {
        ...
        if (data[i] == true) {
            ...
            if (currentPreviewTextLen == 0)
                sprintf(buf, crypt_str("%s"), displayName[i]);
            else
                sprintf(buf, crypt_str(", %s"), displayName[i]);
            ...
            if (textSize.x > multicomboWidth) {
                ...
                sprintf(previewText + currentPreviewTextLen, crypt_str("..."));
                currentPreviewTextLen += (int)strlen(crypt_str("..."));
                break;
            }
    ...
    if (currentPreviewTextLen > 0)
        ...
        sprintf(previewText, crypt_str("None"));

    bool isDataChanged = false;

    if (ImGui::BeginCombo(name.c_str(), previewText)) {

        for (int i = 0; i < dataSize; i++) {
            sprintf(buf, displayName[i]);

            if (ImGui::Selectable(buf, (bool*)&data[i], ImGuiSelectableFlags_DontClosePopups))
                isDataChanged = true;
        }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags, bool minimum_damage)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    float deltatime = 10.f * ImGui::GetIO().DeltaTime;

    static std::map <ImGuiID, slider_animation> animation;
    auto it_animation = animation.find(id);
    if (it_animation == animation.end())
    {
        animation.insert({ id, {false, 0.f} });
        it_animation = animation.find(id);
    }

    if (hovered && g.IO.MouseDown[0])
        it_animation->second.clicked = true;
    else if (!g.IO.MouseDown[0])
        it_animation->second.clicked = false;

    if (it_animation->second.clicked)
    {
        auto active_num = 1.f;

        if (it_animation->second.circle_anim != active_num)
        {
            if (it_animation->second.circle_anim < active_num)
                it_animation->second.circle_anim = ImMin(it_animation->second.circle_anim + deltatime, active_num);
            else if (it_animation->second.circle_anim > active_num)
                it_animation->second.circle_anim = ImMax(active_num, it_animation->second.circle_anim - deltatime);
        }
    }
    else
    {
        ...
        if (it_animation->second.circle_anim != disable_num)
        {
            if (it_animation->second.circle_anim < disable_num)
                it_animation->second.circle_anim = ImMin(it_animation->second.circle_anim + deltatime, disable_num);
            else if (it_animation->second.circle_anim > disable_num)
                it_animation->second.circle_anim = ImMax(disable_num, it_animation->second.circle_anim - deltatime);
        }
    }

    window->DrawList->AddRectFilled(clickable.Min + ImVec2(0.f, 7.f), clickable.Max - ImVec2(0.f, 7.f), ImColor(ImVec4(0.04f, 0.04f, 0.04f, c_menu::get().public_alpha)));

    window->DrawList->AddRectFilled(clickable.Min + ImVec2(0.f, 7.f), grab_bb.Max - ImVec2(10.f, 5.f), ImColor(ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, c_menu::get().public_alpha)));

    window->DrawList->AddCircleFilled(grab_bb.Min + ImVec2(5.f, 6.f), 6.f + it_animation->second.circle_anim, ImColor(ImVec4(1.f, 1.f, 1.f, c_menu::get().public_alpha)), 30.f);
...
bool ImGui::SliderInt(const char* label, int* v, int v_min, int v_max, bool minimum_damage, const char* format, ImGuiSliderFlags flags)
{
    return SliderScalar(label, ImGuiDataType_S32, v, &v_min, &v_max, format, flags, minimum_damage);
}
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    if (p_step != NULL)
    {
        ...
        if (InputText(label, buf, IM_ARRAYSIZE(buf), flags)) // PushId(label) + "" gives us the expected ID from outside point of view
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const float size = ImGui::GetWindowWidth() - 20;
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(size, 24));
    const ImRect total_bb(window->DC.CursorPos + ImVec2(0, 18), window->DC.CursorPos + ImVec2(size, 44));
    ...
    it_hover->second = ImClamp(it_hover->second + (2.15f * ImGui::GetIO().DeltaTime * (hovered || IsItemActive() ? c_menu::get().public_alpha : -1.f)), 0.0f, 1.f);
    // Render frame  
    if (!is_multiline)
    {
        RenderNavHighlight(frame_bb, id);
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(ImVec4(0.04f, 0.04f, 0.04f, c_menu::get().public_alpha)), 3.f);
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, it_hover->second)), 3.f);
    }
   ...
   // FIXME: We could remove the '&& render_cursor' to keep rendering selection when inactive.
    if (render_cursor || render_selection)
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, ImColor(1.f, 1.f, 1.f, c_menu::get().public_alpha * ImMax(0.5f, it_hover->second)), buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    ...
    else
    {
        ...
        {
            ImU32 col = GetColorU32(is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos, ImColor(1.f, 1.f, 1.f, c_menu::get().public_alpha * ImMax(0.5f, it_hover->second)), buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    }
...
bool ImGui::ColorEdit(const char* label, Color* newcolor, ImGuiColorEditFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton(label, col_v4, flags))
        {
            if (!(flags & ImGuiColorEditFlags_NoPicker))
            {
                ...
                OpenPopup("picker");
                SetNextWindowPos(window->DC.LastItemRect.GetBR() + ImVec2(-21, style.ItemSpacing.y)); // GHETTO CODE ALERT
            }
        }
        if (!(flags & ImGuiColorEditFlags_NoOptions))
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col) {
    ...
    bool alpha_bar = (flags & ImGuiColorEditFlags_NoAlpha) ? false : true;
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, bool simple)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const float width_size = simple ? GetFrameHeight() : GetWindowWidth() - 20;
    const ImRect rect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(width_size, 20));
    ...
    ImVec4 col_rgb = col;
    col_rgb.w = math::clamp(col_rgb.w, 0.f, c_menu::get().public_alpha);
  
    if (flags & ImGuiColorEditFlags_InputHSV)
        ColorConvertHSVtoRGB(col_rgb.x, col_rgb.y, col_rgb.z, col_rgb.x, col_rgb.y, col_rgb.z);

    ImVec4 col_rgb_without_alpha(col_rgb.x, col_rgb.y, col_rgb.z, c_menu::get().public_alpha);

    if (!simple)
        window->DrawList->AddText(rect.Min + ImVec2(0, 4), ImColor(ImVec4(1.f, 1.f, 1.f, c_menu::get().public_alpha)), desc_id);
    ...
    window->DrawList->AddRect(clickable.Min, clickable.Max, ImColor(ImVec4(0.f, 0.f, 0.f, c_menu::get().public_alpha)), 3.f);
    ...
    if (g.ActiveId == id && !(flags & ImGuiColorEditFlags_NoDragDrop) && BeginDragDropSource())
    {
        ...
        ColorButton(desc_id, col, flags, true);
        SameLine();
        TextEx("Color");
        EndDragDropSource();
    }
...
void ImGui::ColorTooltip(const char* text, const float* col, ImGuiColorEditFlags flags)
{
    ...
    ColorButton("##preview", cf, (flags & (ImGuiColorEditFlags__InputMask | ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf)) | ImGuiColorEditFlags_NoTooltip);
    SameLine();
...
bool ImGui::CustomSelectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    const ImVec4 text_hov = ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, 1.f);
    const ImVec4 text_dis = ImVec4(251 / 255.f, 251 / 255.f, 251 / 255.f, 1.f);
    float deltatime = 1.5f * ImGui::GetIO().DeltaTime;
    ...
    PushStyleColor(ImGuiCol_Text, ImVec4(it_hcolor->second.x, it_hcolor->second.y, it_hcolor->second.z, c_menu::get().public_alpha));
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    const ImVec4 text_act = ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, 1.f);
    const ImVec4 text_hov = ImVec4((g_cfg.menu.menu_theme.r() / 255.f) - 0.10f, (g_cfg.menu.menu_theme.g() / 255.f) - 0.10f, (g_cfg.menu.menu_theme.b() / 255.f) - 0.10f, 1.f);
...
bool ImGui::sub(const char* label, bool selected)
{
    ...
    if (selected)
    {
        window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(44, 52, 62), 5);
...
bool ImGui::SubSubTab(const char* label, ImFont* font, bool selected)
{
    ImGui::PushFont(font);
    ...
    static std::map<ImGuiID, ImVec4> text_animation;
    auto it_animation = text_animation.find(id);
    if (it_animation == text_animation.end())
    {
        text_animation.insert({ id, text_dis });
        it_animation = text_animation.find(id);
    }

    if (hovered || held || selected)
    {
        ImVec4 to = text_act;
        if (it_animation->second.x != to.x)
        {
            if (it_animation->second.x < to.x)
                it_animation->second.x = ImMin(it_animation->second.x + deltatime, to.x);
            else if (it_animation->second.x > to.x)
                it_animation->second.x = ImMax(to.x, it_animation->second.x - deltatime);
        }

        if (it_animation->second.y != to.y)
        {
            if (it_animation->second.y < to.y)
                it_animation->second.y = ImMin(it_animation->second.y + deltatime, to.y);
            else if (it_animation->second.y > to.y)
                it_animation->second.y = ImMax(to.y, it_animation->second.y - deltatime);
        }

        if (it_animation->second.z != to.z)
        {
            if (it_animation->second.z < to.z)
                it_animation->second.z = ImMin(it_animation->second.z + deltatime, to.z);
            else if (it_animation->second.z > to.z)
                it_animation->second.z = ImMax(to.z, it_animation->second.z - deltatime);
        }
    }
    else
    {
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(it_animation->second.x, it_animation->second.y, it_animation->second.z, c_menu::get().public_alpha));
