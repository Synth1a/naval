...
namespace ImGui
{
    ...
    IMGUI_API bool          BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags, bool is_for_listbox = false);
    ...
    IMGUI_API bool Tab(const char* label, const char* icon, bool selected, bool is_for_visual = false); // is_for_visual is temporary.
    IMGUI_API bool SubSubTab(const char* label, ImFont* font, bool selected);
    IMGUI_API bool sub(const char* label, bool selected);
    
    IMGUI_API void          Keybind(const char* label, key_bind* key_bind, const char* unique_id);
    IMGUI_API bool          KeybindButton(const char* label, const char* unique_id, key_bind* key_bind, const ImVec2& size_arg, bool clicked, ImGuiButtonFlags flags = 0);
