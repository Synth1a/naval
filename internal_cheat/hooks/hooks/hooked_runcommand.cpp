...
bool __fastcall hooks::hooked_ispaused(void* ecx, void* edx) 
{
	...
	if (_ReturnAddress() == (void*)g_ctx.addresses.return_to.extrapolation)
		return true;
...
bool __stdcall hooks::hooked_inprediction()
{
	...
	if (_ReturnAddress() == (void*)g_ctx.addresses.return_to.maintain_sequence_transitions)
		return true;

	if (_ReturnAddress() == (void*)g_ctx.addresses.return_to.setupbones_timing)
		return false;
...
void WriteUserСmd(void* buf, CUserCmd* incmd, CUserCmd* outcmd)
{
	...
	static auto Fn = (WriteUserCmd_t)g_ctx.addresses.write_user_cmd;
