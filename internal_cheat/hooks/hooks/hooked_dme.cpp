...
void __stdcall hooks::hooked_dme(IMatRenderContext* ctx, const DrawModelState_t& state, const ModelRenderInfo_t& info, matrix3x4_t* bone_to_world)
{
	...
	auto is_player = strstr(name, "models/player") && model_entity && model_entity->is_alive() && (g_cfg.player.type[ENEMY].chams[PLAYER_CHAMS_VISIBLE] || g_cfg.player.type[TEAM].chams[PLAYER_CHAMS_VISIBLE] || g_cfg.player.type[LOCAL].chams[PLAYER_CHAMS_VISIBLE] || g_cfg.player.fake_chams_enable || g_cfg.player.backtrack_chams);
	auto is_ragdoll = strstr(name, "models/player") && model_entity && model_entity->GetClientClass()->m_ClassID == CCSRagdoll && (g_cfg.player.type[ENEMY].ragdoll_chams || g_cfg.player.type[TEAM].ragdoll_chams);
	...
	if (!is_player && !is_ragdoll && !is_weapon && !is_arms && !is_sleeve)
		return original_fn(m_modelrender(), ctx, state, info, bone_to_world);
	...
	if (is_player)
	{
		...
		if (type == ENEMY)
		{
			...
			auto material = util::apply_materials(g_cfg.player.type[ENEMY].chams_type);
			auto double_material = util::apply_materials(6);
			...
			if (material && double_material && !material->IsErrorMaterial() && !double_material->IsErrorMaterial())
			{
				if (g_cfg.player.type[ENEMY].chams[PLAYER_CHAMS_VISIBLE] && g_cfg.player.type[ENEMY].chams[PLAYER_CHAMS_INVISIBLE])
				{
					...
					{
						auto backtrack_material = util::apply_materials(g_cfg.player.backtrack_chams_material);
					...
					{
						auto animated_material = util::apply_materials(9);
		...
		{
			auto material = util::apply_materials(g_cfg.player.type[TEAM].chams_type);
			auto double_material = util::apply_materials(6);
		...
		else if (m_input()->m_fCameraInThirdPerson)
		{
			...
			auto is_scoped = g_ctx.local()->m_hActiveWeapon().Get() ? g_ctx.globals.scoped && g_ctx.local()->m_hActiveWeapon().Get()->is_scopable() && g_ctx.local()->m_hActiveWeapon().Get()->m_zoomLevel() : false;

			if (g_cfg.player.transparency_in_scope && is_scoped)
				alpha_modifier = g_cfg.player.transparency_in_scope_amount;

			auto material = util::apply_materials(g_cfg.player.type[LOCAL].chams_type);
			auto double_material = util::apply_materials(6);
			...
			if (g_cfg.player.fake_chams_enable)
			{
				...
				material = util::apply_materials(g_cfg.player.fake_chams_type);
				...
				m_modelrender()->ForcedMaterialOverride(material);
				original_fn(m_modelrender(), ctx, state, info, g_ctx.animations.local.m_fake_matrix);
				m_modelrender()->ForcedMaterialOverride(nullptr);
	...
	else if (is_ragdoll)
	{
		...
		if (type == ENEMY)
		{
			...
			auto ragdoll_material = util::apply_materials(g_cfg.player.type[ENEMY].ragdoll_chams_material);

			if (ragdoll_material && !ragdoll_material->IsErrorMaterial())
			{
				float ragdoll_color[3] =
				{
					g_cfg.player.type[ENEMY].ragdoll_chams_color[0] / 255.0f,
					g_cfg.player.type[ENEMY].ragdoll_chams_color[1] / 255.0f,
					g_cfg.player.type[ENEMY].ragdoll_chams_color[2] / 255.0f
				};

				m_renderview()->SetBlend(alpha);
				util::color_modulate(ragdoll_color, ragdoll_material);

				ragdoll_material->IncrementReferenceCount(); //-V807
				ragdoll_material->SetMaterialVarFlag(MATERIAL_VAR_IGNOREZ, true);

				m_modelrender()->ForcedMaterialOverride(ragdoll_material);
				original_fn(m_modelrender(), ctx, state, info, bone_to_world);
				m_modelrender()->ForcedMaterialOverride(nullptr);
		...
		{
			auto alpha = (float)g_cfg.player.type[type].ragdoll_chams_color.a() / 255.0f;
			auto ragdoll_material = util::apply_materials(g_cfg.player.type[TEAM].ragdoll_chams_material);

			if (ragdoll_material && !ragdoll_material->IsErrorMaterial())
			{
				float ragdoll_color[3] =
				{
					g_cfg.player.type[TEAM].ragdoll_chams_color[0] / 255.0f,
					g_cfg.player.type[TEAM].ragdoll_chams_color[1] / 255.0f,
					g_cfg.player.type[TEAM].ragdoll_chams_color[2] / 255.0f
				};
	...
	else if (is_weapon)
	{
		...
		auto material = util::apply_materials(g_cfg.esp.weapon_chams_type);
		auto double_material = util::apply_materials(6);
	...
	else if (is_arms || is_sleeve)
	{
		...
		auto material = util::apply_materials(g_cfg.esp.arms_chams_type);
		auto double_material = util::apply_materials(6);
